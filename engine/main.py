import os
import sys
import time
from .config import load_config, save_config
from .state import GameState, save_game, load_game, list_saves
from .ui import clear_screen, type_text, prompt_choice, prompt_save, prompt_load, prompt_settings
from .actions import perform_actions
from .conditions import check_conditions
from .story_loader import load_story

def _normalize_target(gs, target):
    """Нормализует цель перехода, добавляя главу если нужно"""
    if "." not in target and "." in gs.current:
        chapter = gs.current.split(".")[0]
        return f"{chapter}.{target}"
    return target

def _calculate_action_cost(actions):
    """Вычисляет общую стоимость действий в монетах"""
    if not actions:
        return 0
        
    total_cost = 0
    for action in actions:
        if not isinstance(action, dict):
            continue
            
        for key, value in action.items():
            if key == "add_coin":
                cost = int(value)
                if cost < 0:  # Если действие тратит монеты
                    total_cost += abs(cost)
            elif key == "remove_coin":
                total_cost += int(value)
                
    return total_cost

def _can_afford_actions(gs, actions):
    """Проверяет, хватит ли монет для выполнения действий"""
    total_cost = _calculate_action_cost(actions)
    current_coins = gs.stats.get("coins", 0)
    return current_coins >= total_cost

def run(start_scene="prologue.start"):
    """Главный игровой цикл визуальной новеллы."""
    story = load_story()
    gs = GameState(start_scene)

    # === Проверка автозагрузки ===
    config = load_config()
    save_dir = config.get("save_dir", "saves")
    
    # Улучшенная проверка сохранений
    save_files = []
    if os.path.exists(save_dir):
        save_files = [f for f in os.listdir(save_dir) if f.endswith(('.json', '.dat'))]
    
    if save_files:
        ans = input("Загрузить сохранение? (y/n): ").lower().strip()
        if ans == "y":
            if loaded := prompt_load():
                gs = loaded

    while True:
        scene = story.get(gs.current)
        if not scene:
            print(f"[Ошибка: сцена {gs.current} не найдена]")
            # Попытка найти альтернативную сцену
            fallback_scene = gs.current.split('.')[0] + '.start'
            if fallback_scene in story and fallback_scene != gs.current:
                print(f"[Переход к {fallback_scene}]")
                gs.current = fallback_scene
                continue
            return

        # Улучшенная проверка условий - обрабатывает None
        conditions = scene.get("conditions")
        if not check_conditions(gs, conditions):
            print(f"[Условия не выполнены для {gs.current}]")
            # Попытка найти альтернативный путь
            if "fallback" in scene:
                gs.current = _normalize_target(gs, scene["fallback"])
                continue
            return

        clear_screen()
        print(f"--- {gs.current} ---\n")

        # Показываем текст сцены, если он есть
        text = scene.get("text")
        if text:
            type_text(text + "\n")

        # Сохраняем текущую сцену ДО выполнения действий
        previous_scene = gs.current

        # Проверяем, хватит ли монет для действий сцены
        scene_actions = scene.get("actions", [])
        scene_cost = _calculate_action_cost(scene_actions)
        if scene_cost > 0 and not _can_afford_actions(gs, scene_actions):
            print("\n❌ Недостаточно монет для выполнения действий!")
            print(f"   Требуется: {scene_cost} монет")
            print(f"   У вас: {gs.stats.get('coins', 0)} монет")
            input("Нажмите Enter для продолжения...")
            continue

        # ВЫПОЛНЯЕМ ДЕЙСТВИЯ (включая jump)
        perform_actions(gs, scene_actions)

        # Если сцена изменилась после действий (был jump), продолжаем цикл с новой сцены
        if gs.current != previous_scene:
            continue

        # Показываем выборы только если они есть
        choices = [
            ch for ch in scene.get("choices", [])
            if check_conditions(gs, ch.get("conditions"))
        ]

        if not choices:
            print("\n(Конец сцены)")
            # Проверяем автоматический переход если нет выборов
            if "auto_next" in scene:
                gs.current = _normalize_target(gs, scene["auto_next"])
                continue
            return

        user_input = prompt_choice(choices)

        # -------------------------------
        # 💾 Управление настройками/сохранением/загрузкой/выходом
        # -------------------------------
        if user_input == "s":
            prompt_settings()
            input("Enter -> продолжить ")
            continue
        elif user_input == "v":
            prompt_save(gs)
            input("Enter -> продолжить ")
            continue
        elif user_input == "l":
            if loaded := prompt_load():
                gs = loaded
                # После загрузки продолжаем с текущей сцены
                continue
            input("Enter -> продолжить ")
            continue
        elif user_input == "q":
            ans = input("Сохранить перед выходом? (y/n): ").lower()
            if ans == "y":
                prompt_save(gs)
            return

        # -------------------------------
        # Обработка выбора сцены
        # -------------------------------
        try:
            idx = int(user_input) - 1
            if not (0 <= idx < len(choices)):
                raise ValueError
        except ValueError:
            print("Неверный ввод.")
            time.sleep(0.2)
            continue

        selected = choices[idx]

        # Проверяем, хватит ли монет для действий выбора
        choice_actions = selected.get("actions", [])
        choice_cost = _calculate_action_cost(choice_actions)
        if choice_cost > 0 and not _can_afford_actions(gs, choice_actions):
            print("\n❌ Недостаточно монет для этого выбора!")
            print(f"   Требуется: {choice_cost} монет")
            print(f"   У вас: {gs.stats.get('coins', 0)} монет")
            input("Нажмите Enter для продолжения...")
            continue

        # Сохраняем текущую сцену ДО выполнения действий выбора
        previous_scene = gs.current
        perform_actions(gs, choice_actions)

        # Если сцена изменилась после действий выбора, продолжаем цикл
        if gs.current != previous_scene:
            continue

        next_scene = selected.get("next") or selected.get("jump")
        if not next_scene:
            print("\n(Конец пути — нет next/jump)")
            return

        gs.current = _normalize_target(gs, next_scene)
